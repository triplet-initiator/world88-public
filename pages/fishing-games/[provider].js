...
export default function FishingGames({}) {
  ...
  const getGames = async (providerCode) => {
    const query = {
      ...
      category: GAME_CATEGORY.FISHING_GAMES,
    };
            ...
            <div className="flex justify-between">
              <section className="mt-4">
                <Button
                  ...
                  onClick={() => (mobile ? router.push('/') : router.push('/fishing-games'))}
                >
                  กลับ
                </Button>
              </section>
            ...
            <section>
              <Row gutter={{ xs: 8, sm: 16, md: 24 }}>
                {showGame?.map((item, idx) => (
                  <Col key={idx} className="my-4" xxl={4} xl={6} lg={6} md={8} xs={8}>
                    <figure
                      className="game-card"
                      onClick={() => {
                        onClickGame(item);
