...
export default function Sport({}) {
  ...
  const getProvider = async () => {
    const query = {
      category: GAME_CATEGORY.SPORT_BOOK,
    };

    setIsLoading(true);
    await UseProvidersByCategory(query).then((res) => {
      setProviders(res.providers.filter((item) => item.providerCode !== GAME_CATEGORY.UFA));
      setIsLoading(false);
    });
  };
                ...
                {providers?.map((item, idx) => (
                  <Col key={idx} className="my-4" xs={12} sm={12} md={8} lg={8} xl={6}>
                    <figure
                      ...
                      onClick={() => {
                        if (session) {
                          router.push(`/sport/${item.providerCode.toLowerCase()}`);
                        } else {
